    ...
    const fetchData = async () => {
      try {
        const [myCoursesData, allCoursesData] = await Promise.all([
          getCourses({ limit: 100 }), // TODO: Replace with getMyCourses when available
          getCourses({ limit: 6, is_published: true }),
        ]);

        const enrolled = Array.isArray(myCoursesData.courses) ? myCoursesData.courses : [];
        setMyCourses(enrolled);
        setRecentCourses(Array.isArray(allCoursesData.courses) ? allCoursesData.courses : []);
